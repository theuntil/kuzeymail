...
export async function handleDeleteUser(
   ...
   * ┌─ PROFİL SİLİNMİYOR, ANONİMLEŞTİRİLİYOR ⚠️ ─────────────────┐
   * │ Önce "profil satırı hâlâ varsa panel sırayı bozmuştur"    │
   * │ varsayılıyordu ve 409 dönülüyordu. Oysa                    │
   * │ `admin_delete_user` satırı SİLMİYOR: haber ve yorumlardaki│
   * │ yazar bağı kopmasın diye anonimleştirip                    │
   * │ `is_active = false` yapıyor.                                │
   * │                                                              │
   * │ Sonuç: denetim her zaman 409 döndürüyor, auth kaydı hiç   │
   * │ silinmiyordu. Yönetici sildiğini sanıyor, kullanıcı       │
   * │ Supabase'de duruyordu.                                       │
   * │                                                              │
   * │ Doğru denetim `is_active` alanına bakmak.                  │
   * └──────────────────────────────────────────────────────────────┘
   */
  const { data: profil } = await sb
    .from("profiles").select("id, is_active").eq("id", id).maybeSingle();

  /*
   * ⚠ PROFİL YOKSA DA DEVAM.
   * Eski kayıtlarda profil hiç oluşmamış olabilir; auth kaydını
   * silmemek için sebep değil.
   */
  if (profil && profil.is_active !== false) {
    json(res, 409, { error: "profile_still_active" });
    return;
  }
